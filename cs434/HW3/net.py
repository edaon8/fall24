...
# step_size = 0.1
#ss=0.0001: more smooth, value/train accuracy is low and does not improve
#ss=5: very spikey/volatile, value/train accuracy improves over ss=0.0001
#ss=10: surprisingly smooth, value/train accuracy is very low and static around 0.1

# GLOBAL PARAMETERS FOR NETWORK ARCHITECTURE
number_of_layers = 3
# number_of_layers = 5
width_of_layers = 32  # only matters if number of layers > 1
activation = "ReLU" if True else "Sigmoid" 

# Submissions: 
# 1) ReLU:                              L=0.244, TA=93.2, VA=91.8:      0.9293
# 2) Seed=5, epochs=300:                L=0.201, TA=94.6, VA=92.2:      0.9289
# 3) ReLU, layers=5, batch=256:         L=0.125, TA=96.6, VA=91.5:      0.9265
# 4) ReLU, layer-width=32:              L=0.246, TA=93.4, VA=92.4:      0.9345
# 5) ReLU, layer-width=32, layers=3:    L=0.184, TA=95.0, VA=92.0:      0.9308
...
def main():
  ...
  # Q7 Evaluate on Test
  ################################
  predictions = evaluateTest(net, X_test, batch_size)
  
  # save to file
  test_out = np.concatenate(
        (np.expand_dims(np.arange(len(predictions)), axis=1), predictions.astype(int).reshape(-1, 1)),
        axis=1
    )
    
  # add the header
  header = np.array([["id", "digit"]])
  test_out_final = np.vstack((header, test_out.astype(str)))  # Stack header with the data

  # save to CSV file
  filename = 'test_predictions.csv'
  np.savetxt(filename, test_out_final, fmt='%s', delimiter=',')
  logging.info(f"Test predictions saved to '{filename}'")


# Helper function for evaluation
def evaluateTest(model, X_test, batch_size):
  predictions = []
  j = 0
  while j < len(X_test):
