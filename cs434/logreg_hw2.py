...
step_size=0.005
max_iters=50000
# best step sizes/iter size:
# ss=0.005/mi=1000: TA=90.77, VA=91.04 (SUB-1=0.87)
# ss=0.01/mi=1000:  TA=92.92, VA=92.67 (SUB-2=0.88)
# ss=0.05/mi=1000:  TA=96.57, VA=95.86 (SUB-3=0.92)
# ss=0.05/mi=2000:  TA=97.00, VA=96.25 (SUB-4=0.92)
# ss=0.005/mi=5000: TA=94.42, VA=94.16 (SUB-5=0.92)
# ss=0.005/mi=50000: TA=97.42, VA=96.25 (SUB-6=0.91)
...
def main():
    ...
    logging.info(f"Running cross-fold validation for bias case (step_size={step_size}, max_iters={max_iters}):")
    ...
    w, _ = trainLogistic(X_train_bias, y_train)

    X_test_bias = dummyAugment(X_test)
    y_pred_test = X_test_bias @ w >= 0

    # Save to predition submission file
    test_out = np.concatenate((np.expand_dims(np.arange(len(y_pred_test)), axis=1), y_pred_test.astype(int)), axis=1)
    header = np.array([["id","type"]])
    test_out_final = np.vstack((header, test_out.astype(str)))
    np.savetxt('test_predictions.csv', test_out_final, fmt='%s', delimiter=',')
...
#   raise Exception('Student error: You haven\'t implemented the negative log likelihood calculation yet.')
    logits = X @ w
    ll = y * np.log(logistic(logits)) + (1 - y) * np.log(1 - logistic(logits))
    nll = -np.sum(ll)
    return nll
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        w_grad = X.T @ (predictions - y) / len(y)
