...
def main():
    ...
    # train_X, train_y = train_X[(z_scores < 3.5).all(axis=1)], train_y[(z_scores < 3.5).all(axis=1)]
    ...
    # for k in [70,75,80,85]:
      ...
      #######################################
      train_predictions = predict(train_X, train_y, train_X, k)
      train_acc = compute_accuracy(train_y, train_predictions)

      #######################################
      # TODO Compute 4-fold cross validation accuracy
      #######################################
      val_acc, val_acc_var = cross_validation(train_X, train_y, num_folds, k=k)
    ...
    best_k = 70 # change this to match current best k
    # k=75, w/ O: 83.35t, 82.88v (sub 1)
    # k=80, w/o O: 82.94t, 82.33v, z<3
    # k=40, w/o O: 83.36t, 82.89v, z<3 (sub 2)
    # k=40, w/o O: 83.98t, 83.52v, z<3.25 (sub 3)
    # k=80, w/ O: 82.80t, 82.46v
    # k=75, w/ O: 82.91t, 82.53v
    # k=75, w/ O: 82.91t, 82.51v, folds=5 (sub 4)
    # k=75, w/ O: 82.91t, 82.45v, folds=3 (sub 5)
    # k=80, w/ O: 93.96t, 82.01v, folds=4, w/ weights
    # k=70, w/ O: 98.79t, 82.24v, folds=4, w/ weights
    # k=75, w/ O: 98.79t, 82.10v, folds=5, w/ weights

    # Notes:
    # - Removing outliers does not help
    # - 4 folds seems to work best
    # - Weight function may need adjusting
...
def knn_classify_point(examples_X, examples_y, query, k):
    neighbors_idx = get_nearest_neighbors(examples_X, query, k)
    neighbors_labels = examples_y[neighbors_idx].flatten().astype(int)
    predicted_label = np.argmax(np.bincount(neighbors_labels))

    ### Weighted Version ###
    # neighbors_idx = get_nearest_neighbors(examples_X, query, k)
    # distances = np.linalg.norm(examples_X[neighbors_idx] - query, axis=1)
    # weights = 1 / (distances + 1e-5)
    # neighbors_labels = examples_y[neighbors_idx].flatten().astype(int)
    # weighted_vote = np.bincount(neighbors_labels, weights=weights)
    # predicted_label = np.argmax(weighted_vote)
...
def cross_validation(train_X, train_y, num_folds, k=1):
    ...
    for fold in range(num_folds):
        ...
        train_fold_X = np.concatenate([train_X[:val_start], train_X[val_end:]], axis=0)
        train_fold_y = np.concatenate([train_y[:val_start], train_y[val_end:]], axis=0)

        val_predictions = predict(train_fold_X, train_fold_y, val_X, k)
        accuracy = compute_accuracy(val_y, val_predictions)
